...
class RotatedImageSampler extends StdImageSampler {
	...
	public int getPixel(int rowPixel, int colPixel) {
		int newRow = (int) (-Math.sin(rotationCounterClockwise)*colPixel + Math.cos(rotationCounterClockwise)*rowPixel);
		int newCol = (int) (Math.cos(rotationCounterClockwise)*colPixel + Math.sin(rotationCounterClockwise)*rowPixel);
		return this.pixelMatrix[newRow][newCol];
	}
